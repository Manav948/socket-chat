...
// Handles empty fields safely (e.g. "REGISTER|||manav" or "CHAT|kavay|general|hello")
static inline int deserializeFrame(const char* inBuffer, MessageFrame* frame) {
    ...
    // Extract sender (1st to 2nd pipe)
    ...
    // Extract target (2nd to 3rd pipe)
    ...
    // Extract payload (after 3rd pipe)
...
// Send framed message over TCP (4-byte length prefix header + payload)
...
// Receive framed message over TCP (reads 4-byte header then payload)
static inline int recv_framed(SOCKET sd, char* buffer, int max_len) {
    ...
    if (payload_len >= (uint32_t)max_len) return -1;
